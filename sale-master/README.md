...
| | `sale_id = common`（常設） | `種別 = sale`（期間限定） |
| --- | --- | --- |
| 中身 | 5と0の日のリンク、いつでも使えるキャンペーン、5と0の日の汎用コピー | タイムライン、施策別リンク、訴求ポイント、キャンペーン、チケット、バナー、日別スケジュール |
| 表示 | **いつでも** | `開始日` 〜 `終了日` の**期間中だけ** |

判定はページを開いた日（日本時間）で行うので、**セールが終われば自動で引っ込む**。消し忘れが起きない。

タブの2番目も連動して変わる。

| | 1つ目 | 2つ目 | 3つ目 |
| --- | --- | --- | --- |
| セール期間中 | セール情報 | **◯月投稿スケジュール** | 人気ホテル一覧 |
| 期間外 | セール情報（常設のみ） | **5と0の日カレンダー** | 人気ホテル一覧 |

5と0の日カレンダーは毎月5・10・15・20・25・30日を**自動計算**して出す。
マスタに日付を書く必要はなく、`schedule` シートの `sale_id = common` の行に
汎用コピー案を数本置いておけば、それを順番に割り当てて使う。
...
1. `sales` シートに新しい行を1行追加する
   ...
   - `種別` は `sale`、`開始日` と `終了日` を入れる（**この期間だけページに出る**）
   - `開始日` は告知を始める日にしておくとよい
...
| シート | 内容 |
| --- | --- |
| `sales` | 1行＝1セール。`種別` が `common` なら常設、`sale` なら `開始日`〜`終了日` の期間限定 |
...
| `schedule_phases` / `schedule` | 投稿スケジュール。`sale_id = common` の行は5と0の日カレンダー用 |
...
| `always` | 常設「いつでも使えるキャンペーン」 | `common` |
